      ...
      <div className="container">
        <div className="product_inner">
          <div className="row align-items-center mb-5">
            ...
            <div className="col-12 col-lg-6">
              {product?.title?.length > 50 ? <h4>{product?.title?.slice(0, 50)}...</h4> : <h4>{product?.title}</h4>
              }
              <h4>

              </h4>
              {
                product?.price?.length > 0 && product?.price?.map(({ piece, euro }, i) => (
                  <>
                    <h5 className="m-0 pb-1" >€ {euro}-{piece}/ <span>piece</span>{" "}</h5>
                  </>
                ))
              }
              <br />
              <div className="d-flex gap-3">
                ...
                <div>
                  ...
                  <p style={{ fontWeight: "400" }}>{product?.lead_time?.from}-{product?.lead_time?.to} {product?.lead_time?.time}</p>
                </div>
              </div>
            </div>
          </div>
