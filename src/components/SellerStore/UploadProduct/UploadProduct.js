...
const UploadProduct = ({ store }) => {
  ...
  const [priceFields, setPriceFields] = useState([{ piece: '', euro: '' }])

  // --------- multi price ---------
  const addPriceFields = () => {
    let priceNewfield = { piece: '', euro: '' }
  ...
  const addFields = () => {
    const infoNewfield = { name: '', value: '' }
    setInputFields([...inputFields, infoNewfield])
  }

  const changePInfo = (index, event) => {
    const data = [...inputFields];
    data[index][event.target.name] = event.target.value;
    setInputFields(data);
  }
        ...
        <form onSubmit={handleSubmit(handleAddProduct)}>
          <div className="row">
            ...
            <div className="col-12 col-lg-6 ">
              ...
              <div style={{ maxWidth: "471px" }}>
                    ...
                    >
                      ...
                      <div className="d-flex gap-2 align-items-center">
                        ...
                        <div className="d-flex gap-2 align-items-center">
                          ...
                          {
                            priceFields?.length > 1 && <span onClick={() => removePrice(index)}>X</span>
                          }
                        </div>
                      </div>
                    </div>
                ...
                {selectedCheckbox === "ladder" && (
                  <button onClick={addPriceFields} className="add_btn">
                    Add more
                  </button>
                )}
              </div>
            ...
            <div className="col-12 mb-5 pb-3">
                ...
                return (
                  <div key={index} className="input-fields" >
                    <input
                      ...
                      value={input.name}
                      required
                      onChange={event => changePInfo(index, event)}
                    />
                    <input
                      name='value'
                      placeholder='value'
                    ...
                    />
                    {
                      inputFields?.length > 1 && <button type="button"
                        onClick={() => removeFields(index)}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6" width="24" >
              ...
              <button onClick={addFields} className="add_btn">
                Add More..
              </button>
            </div>
            <br />
